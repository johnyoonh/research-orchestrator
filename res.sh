#!/bin/bash
# V20 PhD Research Orchestrator (HTTP Error Checking & Status Alerts)
...
READWISE_TOKEN=$(grep -o '"token": *"[^"]*"' "$WIKI_PATH/.obsidian/plugins/readwise-official/data.json" | cut -d'"' -f4)
...
smart_add_file() {
    local ID=$1; local TARGET=$2; local DEST=$3
    ...
    if [[ "$TARGET" =~ ^https?:// ]]; then
        ...
        local AUTH_HEADER=""
        if [[ "$TARGET" == *"readwise.io"* ]] && [[ -n "$READWISE_TOKEN" ]]; then
            AUTH_HEADER="-H \"Authorization: Token $READWISE_TOKEN\""
        fi

        # Execute curl and capture HTTP status code
        # -w %{http_code} prints the code at the end
        HTTP_STATUS=$(curl -sL $AUTH_HEADER -OJ -w "%{http_code}" "$TARGET")
        ...
        FILE=$(ls -t "$PROJ_PATH/inbox/"* | head -n 1)
        echo "$TARGET" >> "$URL_LOG"
...
case "$1" in
    ...
    add|ad|a)
        [[ "$2" =~ ^[0-9]+$ ]] && smart_add_file "$2" "$3" "$4" || smart_add_file 0 "$2" "$3"
        ;;
    ...
    focus|f|fo)
        [[ "$2" =~ ^[0-9]+$ ]] && ID=$2 || ID=0
        [[ "$2" == "default" ]] && ln -sfn "$DEFAULT_INBOX" "$ACTIVE_INBOX" && exit 0
        PATH=$(get_project_path "$ID"); ln -sfn "$PATH/inbox" "$ACTIVE_INBOX"; echo "🎯 Focused: Project $ID"
        ;;

    move|mv|m)
        ID=0; FILE=$2; DEST=$3; [[ "$2" =~ ^[0-9]+$ ]] && { ID=$2; FILE=$3; DEST=$4; }
    ...
    repair|rp|r)
        [[ "$2" =~ ^[0-9]+$ ]] && ID=$2 || ID=0
        PROJ_PATH=$(get_project_path "$ID"); echo "🛠 Repairing Project $ID..."
    ...
    process_active|p|pa)
        TARGET=$(readlink "$ACTIVE_INBOX"); ID=$(grep ":$(basename $(dirname "$TARGET"))$" "$REGISTRY" | cut -d':' -f1)
        for FILE in "$TARGET"/*; do smart_add_file "$ID" "$FILE"; done
        ;;

    finalize|fi|pub)
        ID=0; DEST=$2; [[ "$2" =~ ^[0-9]+$ ]] && { ID=$2; DEST=$3; }
        PROJ_PATH=$(get_project_path "$ID"); PROJECT=$(get_project_name "$ID")
